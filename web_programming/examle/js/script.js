...
function executeExercise1() {
    ...
    // 1. 四角形の色を変数で宣言してみよう
    //    例: let fillColor = "red";
    let fillColor = "red"; // ← ここに好きな色を設定してください

    // 2. 四角形のX座標、Y座標、幅、高さを変数で宣言してみよう
    //    例: let rectX = 50;
    let rectX = 175;    // ← ここにX座標を設定してください
    let rectY = 70;     // ← ここにY座標を設定してください
    let rectWidth = 50; // ← ここに幅を設定してください
    let rectHeight = 50; // ← ここに高さを設定してください
...
function executeExercise2() {
    ...
    let circleColor;
    if (circleX < canvas.width / 3) { // ← ここに条件を書いてみよう
        circleColor = "blue"; // ← 条件が真の時に実行されるコード
    } else if(circleX < canvas.width / 2) {
        circleColor = "yellow"; // ← 条件が偽の時に実行されるコード
    } else {
        circleColor = "red";
    }
...
function executeExercise3() {
    ...
    for (let i = 1; i <= numberOfStars; i++) { // ← ここを完成させてみよう
